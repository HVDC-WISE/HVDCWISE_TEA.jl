...
@testset "Tests Cases" begin
    ...
    @testset "Test case 01 : Single AC Line" begin
        ...
        # 1 dispatchable generator at bus 1, 1 fixed load at bus 2
        baseMVA = 100
        ...
        if results_1 !== nothing
            ...
            @test output_series_1["1"]["load"]["1"]["pflex"] - output_series_1["1"]["bus"]["2"]["p_slack_up"] ≈ 8/baseMVA atol=1e-3
            @test output_series_1["1"]["bus"]["2"]["p_slack_up"] ≈ 0.0 atol=1e-3
            ...
            @test output_series_1["2"]["load"]["1"]["pflex"] - output_series_1["2"]["bus"]["2"]["p_slack_up"] ≈ 10/baseMVA atol=1e-3
            @test output_series_1["2"]["bus"]["2"]["p_slack_up"] ≈ 3/baseMVA atol=1e-3
            for h=1:2
                @test output_series_1["$h"]["bus"]["1"]["p_slack_up"] ≈ 0.0 atol=1e-3
                @test output_series_1["$h"]["bus"]["1"]["p_slack_down"] ≈ 0.0 atol=1e-3
                @test output_series_1["$h"]["bus"]["2"]["p_slack_down"] ≈ 0.0 atol=1e-3
            end
        else
    ...
    @testset "Test case 02 : Single AC Line with flexible load and non-dispatchable generator" begin
        ...
        # 1 dispatchable generator & 1 ndgen at bus 1, 1 flexible load at bus 2
        baseMVA = 100
        ...
        if results_2 !== nothing
            ...
            @test output_series_2["1"]["load"]["1"]["pflex"] - output_series_2["1"]["bus"]["2"]["p_slack_up"] ≈ 9.0/baseMVA atol=1e-3
            @test output_series_2["1"]["bus"]["2"]["p_slack_up"] ≈ 0.0/baseMVA atol=1e-3
            ...
            @test output_series_2["3"]["load"]["1"]["pflex"] - output_series_2["3"]["bus"]["2"]["p_slack_up"] ≈ 5.0/baseMVA atol=1e-3
            ...
            @test output_series_2["2"]["load"]["1"]["pflex"] - output_series_2["2"]["bus"]["2"]["p_slack_up"] ≈ 7.0/baseMVA atol=1e-3
            @test output_series_2["2"]["load"]["1"]["pred"] ≈ 1.0/baseMVA atol=1e-3
            for h=1:3
                @test output_series_2["$h"]["bus"]["1"]["p_slack_up"] ≈ 0.0 atol=1e-3
                @test output_series_2["$h"]["bus"]["1"]["p_slack_down"] ≈ 0.0 atol=1e-3                
                @test output_series_2["$h"]["bus"]["2"]["p_slack_up"] ≈ 0.0 atol=1e-3
                @test output_series_2["$h"]["bus"]["2"]["p_slack_down"] ≈ 0.0 atol=1e-3
            end
        else
    ...
    @testset "Test case 04 : AC/DC Point to Point + Single AC Line" begin
        ...
        if results_4 !== nothing
            ...
            @test output_series_4["1"]["load"]["1"]["pflex"] - output_series_4["1"]["bus"]["2"]["p_slack_up"] ≈ 8.0/baseMVA atol=1e-3
            @test output_series_4["1"]["bus"]["2"]["p_slack_up"] ≈ 0.0/baseMVA atol=1e-3
            ...
            @test output_series_4["2"]["load"]["1"]["pflex"] - output_series_4["2"]["bus"]["2"]["p_slack_up"] ≈ 10.0/baseMVA atol=1e-3
            @test output_series_4["2"]["bus"]["2"]["p_slack_up"] ≈ 3.0/baseMVA atol=1e-3
            ...
            @test output_series_4["3"]["load"]["1"]["pflex"] - output_series_4["3"]["bus"]["2"]["p_slack_up"] ≈ 5.0/baseMVA atol=1e-3
            @test output_series_4["3"]["bus"]["2"]["p_slack_up"] ≈ 3.0/baseMVA atol=1e-3
            ...
            @test output_series_4["4"]["load"]["1"]["pflex"] - output_series_4["4"]["bus"]["2"]["p_slack_up"] ≈ 7.5/baseMVA atol=1e-3
            @test output_series_4["4"]["bus"]["2"]["p_slack_up"] ≈ 0.5/baseMVA atol=1e-3
            ...
            @test output_series_4["5"]["load"]["1"]["pflex"] - output_series_4["5"]["bus"]["2"]["p_slack_up"] ≈ 7.5/baseMVA atol=1e-3
            @test output_series_4["5"]["bus"]["2"]["p_slack_up"] ≈ 0.5/baseMVA atol=1e-3
            ...
            @test output_series_4["6"]["load"]["1"]["pflex"] - output_series_4["6"]["bus"]["2"]["p_slack_up"] ≈ 5.0/baseMVA atol=1e-3
            @test output_series_4["6"]["bus"]["2"]["p_slack_up"] ≈ 3.0/baseMVA atol=1e-3
            for h=1:6
                @test output_series_4["$h"]["bus"]["1"]["p_slack_up"] ≈ 0.0 atol=1e-3
                @test output_series_4["$h"]["bus"]["1"]["p_slack_down"] ≈ 0.0 atol=1e-3
                @test output_series_4["$h"]["bus"]["2"]["p_slack_down"] ≈ 0.0 atol=1e-3
            end
        else
    ...
    @testset "Test case 05b : 1 storage + 1 generator + 1 load" begin
        # Load and run test case 5
        # 1 fixed load at bus 1, 1 dispatchable generator at bus 2, 1 storage at bus 3
        baseMVA = 100
        ...
        if results_5 !== nothing
            ...
            # t = 1 & 2: Demand=9 and Available=10 -> 1 MW consumed by the storage & 9 MW by the load (no load shedding)
                ...
                @test output_series_5[t]["bus"]["1"]["p_slack_up"] ≈ 0/baseMVA atol=1e-3
            ...
            @test output_series_5["3"]["gen"]["1"]["pg"] ≈ 10/baseMVA atol=1e-3
            @test output_series_5["3"]["bus"]["1"]["p_slack_up"] ≈ 1/baseMVA atol=1e-3
            @test output_series_5["3"]["storage"]["1"]["ps"] ≈ -2/baseMVA atol=1e-3
            # @test output_series_5["1"]["storage"]["1"]["sd"] ≈ 2/baseMVA atol=1e-3  # ps=sc-sd
            for h=1:3
                @test output_series_5["$h"]["bus"]["1"]["p_slack_down"] ≈ 0.0 atol=1e-3
                @test output_series_5["$h"]["bus"]["2"]["p_slack_up"] ≈ 0.0 atol=1e-3
                @test output_series_5["$h"]["bus"]["2"]["p_slack_down"] ≈ 0.0 atol=1e-3
            end
        else
